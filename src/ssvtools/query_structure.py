...
def get_in_body_coordinates(region, coordinate_field_name, template_region, template_coordinate_field_name,
    ...
    of a template vagus scaffold. The coordinates field of the template trunk is written to a memory buffer in a
    template region and loaded back into the region to replace the field of the SSV trunk with the same name as the
    template coordinate field. The values are then used to replace the values in the choosen SSv coordinate field.
    This means that the chosen coordinate field of the SSV trunk now follows the path of the template trunk. The radius
    along the resulting trunk is then scaled to the radius along the original SSV trunk using a unit conversion factor.
    As the SSV now has a rescaled trunk, the in-body position of the SSV branches can be determined from the new trunk
    coordinates.
    ...
    assert elements_along_template_trunk == elements_along_data_trunk, \
        "get_in_body_coordinates. Number of trunk elements in data does not match number of trunk elements in " \
        "template vagus."
